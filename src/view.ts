...
const neighborsOf = (edges: ReadonlyArray<GraphEdge>, id: string): ReadonlySet<string> => {
...
const edgeHighlightOf = (selectedNodeId: Option.Option<string>, edge: GraphEdge): EdgeHighlight =>
	pipe(
		selectedNodeId,
		Option.match({
			onNone: () => 'normal' as const,
...
const findPosition = (
	...
	id: string,
): Option.Option<NodePosition> => Array.findFirst(positions, position => position.id === id)
...
const nodeView = (
	...
	h.g(
		[
			...
			h.Class(highlight === 'neighbor' ? 'cursor-pointer node-neighbor' : 'cursor-pointer'),
		...
		[
			...
			h.ellipse(
				[h.Cx('54'), h.Cy('16'), h.Rx('26'), h.Ry('10'), h.Fill('white'), h.Opacity('0.3')],
				[],
			),
			h.text(
				[
...
const selectedDetailView = (model: Model, h: HtmlBuilder<Message>): Html =>
	pipe(
		...
		Option.match({
			onNone: () => h.p([h.Class('text-sm text-stone-500')], ['Click a node to see its details']),
			onSome: node =>
				h.p([h.Class('text-sm text-stone-900')], [`Selected: ${node.id} (${node.resourceType})`]),
		}),
	)
...
export const view = (model: Model, h: HtmlBuilder<Message>): Document => {
	...
	return {
		...
		body: h.div(
			...
			[
				h.h1([h.Class('mb-1 font-serif text-2xl')], ['The infra serving this page']),
				h.p(
