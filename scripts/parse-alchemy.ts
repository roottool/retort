...
const findStateFilePaths = async (stateRoot: string): Promise<ReadonlyArray<string>> => {
...
const readResourceState = async (path: string): Promise<typeof AlchemyResourceState.Type> => {
...
const main = async (): Promise<void> => {
	if (!existsSync(STATE_ROOT)) {
		console.error(`${STATE_ROOT} not found. Run \`bun alchemy deploy\` first.`)
		process.exit(1)
	}
	...
	const resourcesWithAssets = resources.filter(resource => resource.props?.assets !== undefined)
